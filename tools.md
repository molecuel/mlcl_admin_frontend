# The Tools Used in `mlcl_admin_frontend`

## Introduction

`mlcl_admin_frontend` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `mlcl_admin_frontend` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`mlcl_admin_frontend` uses git to manage its codebase. While in theory you don't have
to use Git once you download `mlcl_admin_frontend`, this project makes the assumption
...
While `mlcl_admin_frontend` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `mlcl_admin_frontend` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `mlcl_admin_frontend`; Grunt is
...
to get started with `mlcl_admin_frontend`. But for those curious or looking to go a
...
Grunt is the engine behind `mlcl_admin_frontend`. It's the magic that makes it move.
...
`mlcl_admin_frontend` comes with a `bower.json` file that looks something like this:
...
in `mlcl_admin_frontend` by default in there to see what I mean.
